    ...
    <section ref={ref} className="relative flex h-screen min-h-[700px] items-end overflow-hidden">
      ...
      <motion.div className="absolute inset-0 z-0" style={{ scale }}>
        <Image src="/images/graduation-denny.webp" alt="Dr. Sunday Okafor" fill priority
          className="object-cover object-[center_30%]" sizes="100vw" />
        {/* Overlays */}
      ...
      <motion.div className="relative z-10 w-full" style={{ y: textY, opacity }}>
        <div className="container pb-[72px]">
          ...
          <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.4 }}
            ...
            <div className="h-px w-9 shrink-0 bg-white/50" />
            <span className="font-title text-[10px] uppercase tracking-[4px] text-white/60">
            </span>
          </motion.div>
          ...
          <div className="mb-1 overflow-hidden">
            <motion.h1 initial={{ y: 120 }} animate={{ y: 0 }} transition={{ duration: 1.1, delay: 0.5, ease: [0.22, 1, 0.36, 1] }}
              className="m-0 font-display text-[clamp(52px,8.5vw,116px)] font-light leading-[0.95] text-white">
              Sunday Chizoba 
            </motion.h1>
          </div>
          <div className="mb-[22px] overflow-hidden">
            <motion.h1 initial={{ y: 120 }} animate={{ y: 0 }} transition={{ duration: 1.1, delay: 0.65, ease: [0.22, 1, 0.36, 1] }}
              className="m-0 font-display text-[clamp(52px,8.5vw,116px)] font-semibold italic leading-[0.95] text-white/[0.92]">
              Okafor
            </motion.h1>
          </div>
            ...
            <div className="relative h-11 min-w-[220px] overflow-hidden">
              {words.map((lines, i) => (
                <motion.span key={lines.join(" ")}
                  ...
                  className={`flex h-11 flex-col justify-center whitespace-nowrap font-title text-[clamp(14px,1.5vw,17px)] uppercase leading-[1.25] tracking-[3px] text-white/90 ${i === 0 ? "relative" : "absolute left-0 top-0"}`}>
                  {lines.map((line) => (
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.2 }}
        ...
        <span className="font-title text-[9px] tracking-[3px] text-white/40 [writing-mode:vertical-rl]">SCROLL</span>
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }}
          className="h-[52px] w-px bg-[linear-gradient(to_bottom,rgba(255,255,255,0.5),transparent)]" />
      </motion.div>
    </section>
  );
}
